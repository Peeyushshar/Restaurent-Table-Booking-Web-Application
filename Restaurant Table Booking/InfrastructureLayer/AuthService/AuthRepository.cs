...
namespace InfrastructureLayer.AuthService
{
    public class AuthRepository : IAuthRepository
    {
        ...
        public async Task<ResponseModel> CreateAsync(ApplicationUser identityUser, string password, string[] role)
        {
            var response = new ResponseModel();
            ...
            if (identityResult.Succeeded)
            {
                ...
                {
                    response.IsSucceeded = true;
                    response.DescriptionMessage = "User registered successfully! Please Login!";
                    response.Data = identityResult;
                    return response;
                }
            }
            response.IsSucceeded = false;
            response.DescriptionMessage = "Something went wrong";
            return response;
        }
    }
}
